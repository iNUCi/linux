...
{
	return intel_scu_ipc_dev_update(NULL, addr, 0, mask);
}

static inline int gpadc_set_bits(u16 addr, u8 mask)
{
	return intel_scu_ipc_dev_update(NULL, addr, 0xff, mask);
}
...
static int msic_gpadc_probe(struct platform_device *ipcdev)
{
	struct gpadc_info *mgi = &gpadc_info;
	int err = 0;
	...
	mgi->dev = &ipcdev->dev;
	mgi->intr = ioremap(0xffff7fc0, 4);
	mgi->irq = platform_get_irq(ipcdev, 0);
...
static void msic_gpadc_remove(struct platform_device *ipcdev)
{
	struct gpadc_info *mgi = &gpadc_info;
