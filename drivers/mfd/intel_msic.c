...
{
	return intel_scu_ipc_dev_ioread8(NULL, reg, val);
}
...
{
	return intel_scu_ipc_dev_iowrite8(NULL, reg, val);
}
...
{
	return intel_scu_ipc_dev_update(NULL, reg, val, mask);
}
...
int intel_msic_bulk_read(unsigned short *reg, u8 *buf, size_t count)
{
	...
	return intel_scu_ipc_dev_readv(NULL, reg, buf, count);
}
...
int intel_msic_bulk_write(unsigned short *reg, u8 *buf, size_t count)
{
	...
	return intel_scu_ipc_dev_writev(NULL, reg, buf, count);
}
...
static int intel_msic_probe(struct platform_device *pdev)
{
	...
	ret = intel_scu_ipc_dev_ioread8(NULL, INTEL_MSIC_ID0, &id0);
	if (ret) {
		dev_err(&pdev->dev, "failed to identify the MSIC chip (ID0)\n");
		return -ENXIO;
	}

	ret = intel_scu_ipc_dev_ioread8(NULL, INTEL_MSIC_ID1, &id1);
	if (ret) {
...
static void intel_msic_remove(struct platform_device *pdev)
{
