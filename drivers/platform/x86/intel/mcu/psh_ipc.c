...
static ssize_t psh_msg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "Last ia2psh command with msg: %#x\nparam: %#x\n",
...
static DEVICE_ATTR_RW(psh_msg);

static ssize_t psh_ch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sysfs_emit(buf, "Last psh channel: %d\n", psh_ch);
}
...
static DEVICE_ATTR_RW(psh_ch);

static ssize_t ia2psh_cmd_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size)
{
...
static DEVICE_ATTR_WO(ia2psh_cmd);
...
static DEFINE_NOIRQ_DEV_PM_OPS(psh_ipc_drv_pm_ops, psh_ipc_suspend_noirq, psh_ipc_resume_noirq);
...
static struct pci_driver psh_ipc_driver = {
	.name = "intel_psh_ipc",
	.driver = {
		.pm = pm_sleep_ptr(&psh_ipc_drv_pm_ops),
		.dev_groups = psh_attr_groups,
	},
