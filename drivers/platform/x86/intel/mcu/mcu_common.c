...
/*
 * Intel MCU common interface file
 *
...
static int do_setup_ddr(struct mcu_data *data)
{
	...
	if (!request_firmware(&fw_entry, fname, data->dev)) {
		...
		if (fw_entry->size > APP_IMR_SIZE) {
			...
			memcpy(mcu->ddr[0], fw_entry->data, fw_entry->size);
...
static ssize_t intel_mcu_tty_write(struct tty_struct *tty, const u8 *buf, size_t count)
{
...
static unsigned int intel_mcu_tty_write_room(struct tty_struct *tty)
{
...
static int mem_alloc(struct pci_dev *pdev, uintptr_t *phy_addr, void **virt_addr, int bar)
{
	int ret = 0;
	...
	mem = memremap(start, len, MEMREMAP_WT);
	if (!mem) {
...
static int mcu_platform_probe(struct mcu *mcu)
{
	int ret, i;
	struct mcu_data *data;
	u8 *base;

	mcu->tty_driver = tty_alloc_driver(INTEL_MCU_TTY_MINORS, TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV);
	if (IS_ERR(mcu->tty_driver))
		return dev_err_probe(mcu->dev, PTR_ERR(mcu->tty_driver), "fail to alloc tty driver\n");

	mcu->tty_driver->name = "ttymcu";
	mcu->tty_driver->major = INTEL_MCU_TTY_MAJOR;
	mcu->tty_driver->minor_start = 0;
	mcu->tty_driver->type = TTY_DRIVER_TYPE_SERIAL;
	mcu->tty_driver->subtype = SERIAL_TYPE_NORMAL;
	mcu->tty_driver->init_termios = tty_std_termios;
	mcu->tty_driver->init_termios.c_cflag = B9600 | CS8 | CREAD | HUPCL | CLOCAL;
	mcu->tty_driver->init_termios.c_ispeed = 38400;
	mcu->tty_driver->init_termios.c_ospeed = 38400;
	mcu->tty_driver->init_termios.c_iflag = 0;
	mcu->tty_driver->init_termios.c_oflag = 0;
	mcu->tty_driver->init_termios.c_lflag = 0;
	tty_set_operations(mcu->tty_driver, &intel_mcu_ops);

	ret = tty_register_driver(mcu->tty_driver);
	if (ret) {
		dev_err(mcu->dev, "fail to register tty driver\n");
		goto tty_reg_fail;
	}
	...
	for (i = INTEL_MCU_TTY_MINORS - 1; i >= 0; i--) {
		...
		if (data == NULL) {
			dev_err(mcu->dev, "fail to alloc mcu data\n");
			goto data_alloc_fail;
		}

		data->index = i;
		tty_port_init(&data->port);
		data->dev = tty_port_register_device(&data->port, mcu->tty_driver, i, mcu->dev);
		mcu_table[i] = data;
		data->mcu = mcu;
	...
	ret = sysfs_create_group(&mcu->dev->kobj, &intel_mcu_tty_attribute_group);
	if (ret) {
		pr_err("failed to create the mdbg sysfs attributes\n");
		sysfs_remove_group(&mcu->dev->kobj, &intel_mcu_tty_attribute_group);
		goto data_alloc_fail;
	}
...
data_alloc_fail:
	for (i = 0; i < INTEL_MCU_TTY_MINORS; i++)
		kfree(mcu_table[i]);
tty_reg_fail:
	tty_driver_kref_put(mcu->tty_driver);
	return ret;
}

static void mcu_platform_remove(struct mcu *mcu)
{
	int i;
...
static int intel_mcu_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	pci_set_drvdata(pdev, mcu);

	return mcu_platform_probe(mcu);
...
{
	struct mcu *mcu = pci_get_drvdata(pdev);

	mcu_platform_remove(mcu);
...
MODULE_AUTHOR("Lei Wen <lei.wen@intel.com>");
MODULE_DESCRIPTION("Intel mcu common control interface");
MODULE_LICENSE("GPL");
